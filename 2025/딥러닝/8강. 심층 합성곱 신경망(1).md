...
- VGGNet이란?
  ...
  - AlexNet에서 사용했던 A구조와 A-LRN 모델 성능 비교헀을 때, 정확도 향상에 큰 효과 없음
  - 더 깊은 층 활용하는 구조에는 LRN 적용하지 않음
  - 층이 깊어지면서 에러율 감소 확인(4 > 7> 11> ...)
- VGGNet 16(D) 개념도
  - 입력된 데이터의 크기는 224x224x3
  - 합성곱 층에서 패딩은 SAME(1), 스트라이드는 1로 설정
  - 모든 최대 풀링 크기는 2x2, 스트라이드 2, 이미지 크기 1/4로 줄어듦



### 2. VGGNet의 특징

- VGGNet16 모델은 3x3 크기의 필터(Conv3)만 사용
  - 10x10 데이터에 7x7 필터 적용시, 한번의 합성곱 연산으로 4x4 특징 맵 만듦
  - 즉, 49개(7*7) 가중치 가짐
- 가중치는 학습에 의해 최적화됨.
  - 가중치가 적다는건 연산량 감소로 인해 학습 속도 높아지고, 메모리 사용량 줄어듦
  - 신경망의 층수가 깊어지면서, 활성함수를 통한 비선형성이 높아지므로 학습을 잘 할 수 있도록 영향 미침
